...
from xui_api import create_trial_inbound
from yookassa_pay import create_payment_link, check_payment_status
from database import save_payment, update_payment
...
main_menu = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="📱 Как подключить"), KeyboardButton(text="⚙️ Техподдержка")],
...
def register_handlers(dp):
    ...
    async def check_payment(callback: types.CallbackQuery):
        ...
        if status == 'succeeded':
            ...
            await callback.message.edit_text(
                ...
                "Используйте команду 👤 Мой профиль для просмотра деталей подписки.",
                parse_mode="Markdown"
            )
    ...
    async def how_to_connect(message: types.Message):
        await message.answer(
            ...
            "• Hiddify (iOS/Android) - самое удобное\n"
            "• v2rayNG (Android)\n"
            "• V2Box (iOS/Android)\n\n"
            "🔹 *Шаг 2: Получите ссылку подписки*\n"
            "Нажмите '🚀 Пробная подписка' или купите тариф в '💎 Купить подписку'\n\n"
            "🔹 *Шаг 3: Импортируйте конфигурацию*\n"
            "• Скопируйте ссылку подписки\n"
            "• Откройте приложение\n"
            "• Добавьте подписку через \"Импорт\"\n"
            "• Выберите сервер и подключитесь\n\n"
            "✅ Готово! Наслаждайтесь свободой в интернете! 🌍",
            parse_mode="Markdown"
        )
